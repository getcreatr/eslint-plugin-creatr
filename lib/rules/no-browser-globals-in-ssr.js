    ...
    function isInsideFunctionCall(node, functionNames) {
      ...
      while (parent) {
        if (parent.type === 'CallExpression') {
          ...
          // Handle direct function calls (useEffect)
          if (parent.callee.type === 'Identifier') {
            callName = parent.callee.name;
          }
          // Handle member expressions (React.useEffect)
          else if (
            parent.callee.type === 'MemberExpression' &&
            parent.callee.property.type === 'Identifier'
          ) {
            callName = parent.callee.property.name;
          }
          
          if (callName && functionNames.includes(callName)) {
            return true;
          }
        }
